...
| Admin Tools | Done | Catalogs, command history, CLI-backed player actions, and broadcast/shutdown are wired. | `admin-server/src/server.js`, `admin-server/src/runner.js`, `admin-server/src/rmq.js`, `web/src/App.tsx` | None for current supported actions. | Yes. |
...
| Broadcast | Done | Publishes verified RedBlink `ServiceBroadcast` envelopes to `dune-rmq-game` `heartbeats/notifications`. | `admin-server/src/rmq.js`, `admin-server/src/server.js`, `web/src/api/admin.ts` | None for current broadcast/shutdown paths. | Yes. |
